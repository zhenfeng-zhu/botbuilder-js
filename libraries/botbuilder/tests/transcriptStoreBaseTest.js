...
const { ActivityTypes } = require("../../botbuilder-core/lib");
...
function promiseSeq(promiseFuncArray) {
	return promiseFuncArray.reduce((chain, promiseFunc) => {
		return chain.then(chainResult =>
			promiseFunc().then(currentResult => [...chainResult, currentResult])
		);
	}, Promise.resolve([]));
}
...
function resolvePromises(promiseFuncArray, useParallel = true) {
	return useParallel
		? promiseParallel(promiseFuncArray)
		: promiseSeq(promiseFuncArray);
}
...
exports._badArgs = function _badArgs(store) {
	var assertPromise = (promiseFunc, errMessage) =>
...
exports._logMultipleActivities = function _logMultipleActivities(
	store,
	useParallel = true
) {
	...
	return resolvePromises(writes, useParallel).then(() => {
		...
		return Promise.all([
			...
			store
				.getTranscriptActivities("bogus", conversationId)
				.then(pagedResult => {
			...
			store
				.getTranscriptActivities("test", conversationId)
				.then(pagedResult => {
					assert(!pagedResult.continuationToken);
			...
			store
				.getTranscriptActivities(
					...
					conversationId,
					null,
					new Date(start.getTime() + 60000 * 5)
				)
				.then(pagedResult => {
					assert.equal(
						activities.length / 2,
...
exports._deleteTranscript = function _deleteTranscript(
	store,
	useParallel = true
) {
	...
	return resolvePromises(writes, useParallel).then(() => {
		return Promise.all([
			...
			// test B
			store
				.getTranscriptActivities("test", conversationId2)
				.then(pagedResult => {
...
exports._getTranscriptActivities = function _getTranscriptActivities(
	store,
	useParallel = true
) {
		...
		return promiseSeq(
			...
			.then(async () => {
				...
				activities.forEach(activity =>
					assert(seen.includes(activity.id))
				);
				resolve();
			})
...
exports._getTranscriptActivitiesStartDate = function _getTranscriptActivitiesStartDate(
	store,
	useParallel = true
) {
	return new Promise((resolve, reject) => {
		...
		return promiseSeq(
			...
			.then(async () => {
				...
				activities
					.filter(
						a => a.timestamp.getTime() >= referenceDate.getTime()
					)
...
exports._listTranscripts = function _listTranscripts(
	store,
	useParallel = true
) {
	return new Promise((resolve, reject) => {
		...
		var activities = [].concat(
			...conversationIds.map(cId => createActivities(cId, start, 1))
		);
		...
		return promiseSeq(
			...
			.then(async () => {
				...
				var seen = [];
				do {
					pagedResult = await store.listTranscripts(
						"test",
						pagedResult.continuationToken
					);
					assert(pagedResult);
