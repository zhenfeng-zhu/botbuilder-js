...
const { TurnContext } = require("../../botbuilder-core/lib");
const { ChannelValidation } = require("../../botframework-connector/lib");
...
const incomingMessage = TurnContext.applyConversationReference(
	{ text: "test", type: "message" },
	reference,
	true
);
const outgoingMessage = TurnContext.applyConversationReference(
	{ text: "test", type: "message" },
	reference
);
const incomingInvoke = TurnContext.applyConversationReference(
	{ type: "invoke" },
	reference,
	true
);
...
class AdapterUnderTest extends BotFrameworkAdapter {
	...
	testAuthenticateRequest(request, authHeader) {
		return super.authenticateRequest(request, authHeader);
	}
		...
		return {
			conversations: {
				replyToActivity: (conversationId, activityId, activity) => {
					assert(
						conversationId,
						`replyToActivity() not passed conversationId.`
					);
					assert(
						activityId,
						`replyToActivity() not passed activityId.`
					);
					assert(activity, `replyToActivity() not passed activity.`);
					return this.failOperation
						? Promise.reject(new Error(`failed`))
						: Promise.resolve({ id: "5678" });
				},
				sendToConversation: (conversationId, activity) => {
					assert(
						conversationId,
						`sendToConversation() not passed conversationId.`
					);
					assert(
						activity,
						`sendToConversation() not passed activity.`
					);
					return this.failOperation
						? Promise.reject(new Error(`failed`))
						: Promise.resolve({ id: "5678" });
				...
				updateActivity: (conversationId, activityId, activity) => {
					assert(
						conversationId,
						`updateActivity() not passed conversationId.`
					);
					assert(
						activityId,
						`updateActivity() not passed activityId.`
					);
				...
				deleteActivity: (conversationId, activityId) => {
					assert(
						conversationId,
						`deleteActivity() not passed conversationId.`
					);
					assert(
						activityId,
						`deleteActivity() not passed activityId.`
					);
					return this.failOperation
						? Promise.reject(new Error(`failed`))
						: Promise.resolve();
				...
				createConversation: parameters => {
					assert(
						parameters,
						`createConversation() not passed parameters.`
					);
					return this.failOperation
						? Promise.reject(new Error(`failed`))
						: Promise.resolve({
								id: "convo2",
								serviceUrl: this.newServiceUrl
						  });
				}
			}
		};
...
class MockResponse {
	...
	send(body) {
		...
		assert(
			this.statusCode !== undefined,
			`response.end() called before response.send().`
		);
		this.ended = true;
	}
}
...
describe(`BotFrameworkAdapter`, function() {
	...
	it(`should processActivity().`, function(done) {
		...
		const adapter = new AdapterUnderTest();
		adapter
			.processActivity(req, res, context => {
	...
	it(`should check timestamp in processActivity() sent as body.`, function(done) {
		...
		adapter
			.processActivity(req, res, context => {
				...
				assert(
					context.activity.timestamp instanceof Date,
					`'context.activity.timestamp' is not a date`
				);
				assert.equal(
	...
	it(`should continueConversation().`, function(done) {
		...
		const adapter = new AdapterUnderTest();
		adapter
			.continueConversation(reference, context => {
				assert(context, `context not passed.`);
				...
				assert(
					context.activity.from &&
						context.activity.from.id === reference.user.id,
					`request has invalid from.id.`
				);
				assert(
					context.activity.recipient &&
						context.activity.recipient.id === reference.bot.id,
	...
	it(`should createConversation().`, function(done) {
		...
		const adapter = new AdapterUnderTest();
		adapter
			.createConversation(reference, context => {
				assert(context, `context not passed.`);
				assert(context.activity, `context has no request.`);
				assert(
					context.activity.conversation &&
						context.activity.conversation.id === "convo2",
	...
	it(`should fail to createConversation() if serviceUrl missing.`, function(done) {
		...
		adapter
			.createConversation(bogus, context => {
	...
	it(`should deliver a single activity using sendActivities().`, function(done) {
		...
		adapter.sendActivities(context, [outgoingMessage]).then(responses => {
			assert(
				...
				`array of responses not returned.`
			);
			assert(
				responses.length === 1,
				`invalid number of responses returned.`
			);
	...
	it(`should deliver multiple activities using sendActivities().`, function(done) {
		...
		const context = new TurnContext(adapter, incomingMessage);
		adapter
			.sendActivities(context, [outgoingMessage, outgoingMessage])
			.then(responses => {
				assert(
					Array.isArray(responses),
					`array of responses not returned.`
	...
	it(`should fail to sendActivities().`, function(done) {
		...
		adapter.sendActivities(context, [cpy]).then(
			responses => {
				assert(false, `shouldn't succeed`);
			},
			err => {
	...
	it(`should fail to updateActivity() if serviceUrl missing.`, function(done) {
		const adapter = new AdapterUnderTest();
		const context = new TurnContext(adapter, incomingMessage);
		const cpy = Object.assign({}, incomingMessage, {
			serviceUrl: undefined
		});
		adapter.updateActivity(context, cpy).then(
			responses => {
				assert(false, `shouldn't succeed`);
			},
			err => {
	...
	it(`should fail to deleteActivity() if serviceUrl missing.`, function(done) {
		...
		adapter.deleteActivity(context, cpy).then(
			responses => {
				assert(false, `shouldn't succeed`);
			},
			err => {
	...
	it(`should create a User-Agent header with the same info as the host machine.`, function(done) {
		...
		const userAgent =
			...
			")";
		assert(
			userAgentHeader.includes(userAgent),
			`ConnectorClient doesn't have user-agent header created by BotFrameworkAdapter or header is incorrect.`
		);
		done();
	});
	...
	it(`should set openIdMetadata property on ChannelValidation`, function(done) {
		...
		const adapter = new BotFrameworkAdapter({
			openIdMetadata: testEndpoint
		});
		assert(
			testEndpoint === ChannelValidation.OpenIdMetadataEndpoint,
	...
	it(`should throw error if missing serviceUrl in deleteConversationMember()`, async function() {
		try {
			...
			assert(
				...
				`expected "BotFrameworkAdapter.deleteConversationMember(): missing serviceUrl" Error message, not "${
					err.message
				}"`
			);
			return;
	...
	it(`should throw error if missing conversation in deleteConversationMember()`, async function() {
		try {
			...
			assert(
				...
				`expected "BotFrameworkAdapter.deleteConversationMember(): missing conversation or conversation.id" Error message, not "${
					err.message
				}"`
			);
			return;
	...
	it(`should throw error if missing conversation.id in deleteConversationMember()`, async function() {
		try {
			const adapter = new AdapterUnderTest();
			await adapter.deleteConversationMember({
				activity: { serviceUrl: "https://test.com", conversation: {} }
			});
		} catch (err) {
			assert(
				err.message ===
	...
	it(`should throw error if missing serviceUrl in getActivityMembers()`, async function() {
		try {
			...
			assert(
				...
				`expected "BotFrameworkAdapter.getActivityMembers(): missing serviceUrl" Error message, not "${
					err.message
				}"`
			);
			return;
	...
	it(`should throw error if missing conversation in getActivityMembers()`, async function() {
		try {
			...
			assert(
				...
				`expected "BotFrameworkAdapter.getActivityMembers(): missing conversation or conversation.id" Error message, not "${
					err.message
				}"`
			);
			return;
	...
	it(`should throw error if missing activityId in getActivityMembers()`, async function() {
		try {
			...
			assert(
				...
				`expected "BotFrameworkAdapter.getActivityMembers(): missing both activityId and context.activity.id" Error message, not "${
					err.message
				}"`
			);
			return;
	...
	it(`should throw error if missing serviceUrl in getConversationMembers()`, async function() {
		try {
			...
			assert(
				...
				`expected "BotFrameworkAdapter.getConversationMembers(): missing serviceUrl" Error message, not "${
					err.message
				}"`
			);
			return;
	...
	it(`should throw error if missing conversation in getConversationMembers()`, async function() {
		try {
			...
			assert(
				...
				`expected "BotFrameworkAdapter.getConversationMembers(): missing conversation or conversation.id" Error message, not "${
					err.message
				}"`
			);
			return;
	...
	it(`should throw error if missing from in getUserToken()`, async function() {
		try {
			...
			assert(
				...
				`expected "BotFrameworkAdapter.getUserToken(): missing from or from.id" Error message, not "${
					err.message
				}"`
			);
			return;
	...
	it(`should throw error if missing from in signOutUser()`, async function() {
		try {
			...
			assert(
				...
				`expected "BotFrameworkAdapter.signOutUser(): missing from or from.id" Error message, not "${
					err.message
				}"`
			);
			return;
	...
	it(`should throw error if missing from in signOutUser()`, async function() {
		try {
			...
			assert(
				...
				`expected "BotFrameworkAdapter.getAadTokens(): missing from or from.id" Error message, not "${
					err.message
				}"`
			);
			return;
